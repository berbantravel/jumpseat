...
function CheckoutContent() {
  ...
  const generateRefNo = () => {
    ...
    const packageId = selectedPackage?.id ? selectedPackage.id : "Unknown";
  ...
  const handleQuantityChange = (e) => {
    ...
    setSelectedPackage((prev) => ({ ...prev, quantity: value }));
  };

  const initiatePayment = async (productDetails) => {
    try {
      localStorage.setItem("USER_INFORMATION", JSON.stringify(formData));
      setIsProcessing(true);
  
      const subtotal = selectedPackage ? selectedPackage.price * selectedPackage.quantity : 0;
  ...
  const handleBuyNow = async (packageId, title, quantity) => {
    try {
      if (!selectedPackage) {
      ...
      const productDetails = {
        name: selectedPackage.title || "Unknown",
        description: `1 sim ${selectedPackage.title || "Unknown"}`,
      };
        ...
        <div className="border rounded-lg p-5 shadow-md bg-white">
         ...
         <PaymentMethodSection onPaymentMethodSelect={handlePaymentMethodSelect} />
          ...
          <div className="mt-10 border-t border-gray-200 py-6">
            <button
                type="button"
                onClick={() => handleBuyNow(selectedPackage.planId, selectedPackage.title, selectedPackage.quantity)}    
                className={`w-full rounded-md border border-transparent px-4 py-3 text-base font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ff9e39] focus:ring-offset-2 focus:ring-offset-gray-50 bg-[#ff9e39] bg-gray-400 hover:bg-[#ff9e39]`}
            >
